...
wget --no-check-certificate -O onefast.sh https://raw.githubusercontent.com/caonimagfw/onefast/master/onefast.sh 
sed -i 's/-p#####/-p#####/' onefast.sh
bash onefast.sh
...
yum -y install wget net-tools bind-utils bridge-utils && yum -y install epel-release && yum -y install unar
